...
EchoServer::EchoServer()
    : m_sockets(), m_obs(new EchoConnectionObserver(this)) {}

EchoServer::~EchoServer() { delete m_obs; }

void EchoServer::removeSocket(Socket *_s) {
    for (std::vector<Socket *>::iterator it = m_sockets.begin();
         it != m_sockets.end(); ++it) {
        if ((*it)->get() == _s->get()) {
            (*it)->close();
            delete (*it);
...
void EchoServer::task() {
    while (1) {
        if (m_sockets.size() > 0 && m_sockets.front() != nullptr) {
            if (m_sockets.front()->hasNewConnection(
            ...
            for (size_t i = 1; i < m_sockets.size(); i++) {
                ...
                if ((_s != nullptr)) {
                    ...
                    case Socket::newData:
                        ...
                        if (readSize >= 0) {
                            log_inst.debug(TAG, "socket(%d) read: %d , %s",
                                           _s->get(), readSize, buf);
