...
void LedcChan::onTimer() {
    ...
    // log_inst.info(TAG, "channel %s, mode %d", this->m_name.c_str(), _mode);
}
...
uint32_t LedcChan::get(mode m) const {
    ...
    float factor =
        Config::repo().get<int>("/actors/channels/config", "factor",
...
LedcChannelFactory::LedcChannelFactory() {
    ledc_timer_config_t ledc_timer = {
        ...
        .freq_hz = static_cast<uint32_t>(
            Config::repo().get<int>("/actors/channels/config", "freq", 50)),
