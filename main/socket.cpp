...
bool Socket::bind(int port, struct in_addr adr) {
	...
	int _b = ::bind(m_socket, (struct sockaddr*) &_serv_addr, sizeof(_serv_addr));
	if (_b < 0) {
...
bool Socket::connect(std::string hostname, unsigned short remotePort, const TimeoutValue &timeout) {
	...
	int connReturn = ::connect(m_socket, (const struct sockaddr*)(addrData), (int) getResults->ai_addrlen);
