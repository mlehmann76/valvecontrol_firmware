...
TaskConfig::TaskConfig(repository &r) :
		m_repo(r) {
}
...
bool TaskConfig::readTask(const std::string &name) {
	if (m_repo.find(name) != m_repo.end()) {
		...
		do {
			...
			_task.m_taskitems.push_back(std::make_shared<Task::TasksItem>(_item));
...
void TaskState::regStateVariables() {
	// state : name, alt-name, running, time, remain time, channel, auto, starttime
...
void TaskState::update(const TaskConfig::Task &task, const TaskConfig::Task::TasksItem &item) {
...
detail::TaskConfig::Task::TasksItem Tasks::NoneTaskItem = detail::TaskConfig::Task::TasksItem();
...
void Tasks::stopTask() {
	...
	if (m_state->running() && (m_nextRequest == "task" || m_nextRequest == m_state->name())) {
		std::lock_guard<std::mutex> lock(m_lock);
		// update channel
	...
	while(m_aexit == false) {
		if (m_activeTask != &NoneTask) {
			...
			if (tdiff >= m_activeItem->m_time) {
				...
				while(_findIter != _iterend && (&**_findIter!=m_activeItem)) {
					++_findIter;
				}
