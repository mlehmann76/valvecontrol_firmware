...
class property {
  public:
    ...
    struct mapped_type_proxy {
        ...
        constexpr mapped_type_proxy(property &_p, const key_type &_n)
            : m_p(_p), m_n(_n) {}

        template <typename T> void operator=(T &&_v) { m_p.set({{m_n, _v}}); }

        mapped_type operator()() const { return m_p.get().at(m_n); }
    ...
    void operator=(const property_base &t) { set(t); }

    property_base operator()() const { return get(); }

    operator property_base() const { return get(); }
