...
class HttpAuth : public iHandler {
  ...
  public:
    ...
    HttpAuth(iHandler *_h, AuthToken _at, HTTPAuthMethod mode = BASIC_AUTH)
        : m_handler(_h), m_response(), m_request(), m_mode(mode),
          m_token(std::move(_at)) {}
  ...
  protected:
    void requestAuth(HTTPAuthMethod mode, const char *realm,
                     const char *failMsg);
